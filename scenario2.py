...
def add_arguments():
    ...
    parser.add_argument('-i', '--input_filename', type=str, default='scenario2.scenic')
...
def main():
    ...
    if simulation: # 'simulate' can return None if simulation fails
        ...
        pickup_data = [(data[1][0], data[1][1], data[1][2]) for data in result['puPos']]
        
        features = ['time', 'ego_x', 'ego_y', 'ego_z', 'pickup_x', 'pickup_y', 'pickup_z']
        trace_data = [(t,) + ed + pd for t, ed, pd in zip(time_data, ego_data, pickup_data)]

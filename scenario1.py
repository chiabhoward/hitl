...
def add_arguments():
    ...
    parser.add_argument('-i', '--input_filename', type=str, default='scenario1.scenic')
...
def main():
    ...
    if simulation: # 'simulate' can return None if simulation fails
        ...
        bicycle_data = [(data[1][0], data[1][1], data[1][2]) for data in result['bicPos']]
        
        features = ['time', 'ego_x', 'ego_y', 'ego_z', 'bicycle_x', 'bicycle_y', 'bicycle_z']
        trace_data = [(t,) + ed + bd for t, ed, bd in zip(time_data, ego_data, bicycle_data)]
